      ...
      <form @submit.prevent="handleSubmit" class="gap-y-5 py-10">
        ...
        <div class="flex mb-10">
          ...
          <div class="px-10 w-2/3 ml-10">
            ...
            <p class="py-2 pre-wrap min-h-[100rem]">{{ content }}</p>
            <div v-for="image in files" :key="image" class="mb-10">
              <img :src="image.imageUrl" class="w-full h-auto" />
            </div>
          </div>
          <!--상세정보 스크롤 따라 내려오도록-->
          <div class="absolute top-[calc(100%-600px)] right-[calc(50%-500px)]">
            <div class="sticky p-8 bg-white text-gray-700 rounded border shadow-md w-96 h-[30rem] overflow-auto">
        ...
        <div class="flex justify-between my-5 mx-7">
          <RouterLink to="/">
            <button class="border border-gray-200 rounded-full px-4 py-1 text-sm hover:bg-gray-200" @click="goToList">목록</button>
          </RouterLink>
          <div class="space-x-3">
